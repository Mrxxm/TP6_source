...
* 设计模式 门面模式 迭代器 反射 类自动加载 路由
* 容器，聊聊对框架的理解：

```
1.组织好框架的目录结构
2.入口文件index.php 处理好类的自动加载(框架层面的自动加载、通过composer的自动加载)
3.实现容器(单例、注册树、反射机制)(继承底层的Countable类，ArrayAccess类)
4.配置文件的加载，tp框架配置文件对性能影响，Yaconf扩展实现配置文件的内存常驻
5.路由处理：路由检测、域名检测、合并分组参数、路由规则匹配、路由动态处理、发起路由调度、实例化控制器、执行控制器方法、返回Response对象、输出内容到终端。(中大型公司不做这么复杂的路由，一般在nginx层配置，框架涉及到是为了解决小型公司的痛点)
6.涉及Model层，MVC
7.涉及核心类库，Cache等使用工厂模式。
```
